...
GtkWidget *create_editor_row(ViewContext *view_context, TrackState *track_state, char odd_even) {
    ...
    */
    ...
    if (track_state->track->type == MIX_TRACK) {
        regions = create_mix_editor_row(view_context, track_state); /* This will load the mix editor row, containing the track cells */
    }
    // Format widgets
    ...
    if (track_state->track->type == MIX_TRACK) gtk_overlay_add_overlay(GTK_OVERLAY(editor_row), regions); /* Only mix tracks supported currently */
...
void *load_region_editor(ViewContext *view_context, GtkWidget **region_editor) {
    ...
    */
    ...
    while (track_state) { /* Loop through all tracks */
        ...
        track_state->region_editor = editor_row; /* Link this widget to the track state */
